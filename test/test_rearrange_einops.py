...
class test_rearrange_examples(unittest.TestCase):
  ...
  def test_simple_transpose(self):
    ...
    y = x.rearrange("h w -> w h")
    np.testing.assert_array_equal(y.numpy(), np.array([[0, 3], [1, 4], [2, 5]]))

  def test_split_dims(self):
    x = Tensor.arange(24).reshape(4, 6)
    y = x.rearrange("(h1 h2) (w1 w2) -> (h1 w1) (h2 w2)", h1=2, h2=2, w1=3, w2=2)
    expected = np.array([[ 0,  1,  6,  7],
                         [ 2,  3,  8,  9],
                         [ 4,  5, 10, 11],
                         [12, 13, 18, 19],
                         [14, 15, 20, 21],
  ...
  def test_ellipsis(self):
    ...
    y = x.rearrange("... h w -> ... (h w)")
    np.testing.assert_array_equal(y.numpy(), x.numpy().reshape(2, 12))

  def test_reduce_dims(self):
    x = Tensor.arange(120).reshape(2, 3, 4, 5)
    y = x.rearrange("a b c d -> (a b) (c d)")
    np.testing.assert_array_equal(y.numpy(), x.numpy().reshape(6, 20))

  def test_expand_dims(self):
    x = Tensor.arange(24).reshape(4, 6)
    y = x.rearrange("h w -> h 1 w 1")
    np.testing.assert_array_equal(y.numpy(), x.numpy().reshape(4, 1, 6, 1))

  def test_complex_rearrange(self):
    x = Tensor.arange(120).reshape(2, 3, 4, 5)
    y = x.rearrange("a (b1 b2) (c1 c2) d -> (a b2 d) b1 (c1 c2)", b1=1, b2=3, c1=2, c2=2)
    expected_shape = (30, 1, 4)
    self.assertEqual(y.shape, expected_shape)

  def test_einops_examples(self):
    # Examples from einops documentation
    x = Tensor.arange(2 * 3 * 4 * 5).reshape(2, 3, 4, 5)
    
    # Example 1: Transpose
    y = x.rearrange("b c h w -> b h w c")
    self.assertEqual(y.shape, (2, 4, 5, 3))

    # Example 2: Flatten
    y = x.rearrange("b c h w -> b (c h w)")
    self.assertEqual(y.shape, (2, 60))

    # Example 3: Depth to Space
    y = x.rearrange("b (c1 c2) h w -> b h (w c1) c2", c1=2)
    self.assertEqual(y.shape, (2, 4, 10, 3))
  ...
  def test_error_cases(self):
    ...
    with self.assertRaises(ValueError):
      x.rearrange("a b c -> d e f")

    # Unknown dimension size
    with self.assertRaises(ValueError):
      x.rearrange("a b c -> (a d) b c")

    # Inconsistent dimension size
    with self.assertRaises(ValueError):
      x.rearrange("a a a -> (a a) a")
