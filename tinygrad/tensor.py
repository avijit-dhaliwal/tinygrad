...
class Tensor:
  ...
  def rearrange(self, pattern: str, **axes_lengths: int) -> 'Tensor':
    """
    Rearranges the tensor according to the given pattern.

    Args:
      pattern (str): The rearrangement pattern.
      **axes_lengths: Lengths of axes if not present in the tensor shape.
    ...
    Examples:
      ...
      >>> x.rearrange("h w -> w h").numpy()
      array([[0, 3],
             [1, 4],
             [2, 5]])
    """
    def parse_pattern(p: str) -> List[str]:
      return [dim for dim in p.replace('(', ' ( ').replace(')', ' ) ').split() if dim != '->']

    def parse_bracket_structure(dims: List[str]) -> List[List[str]]:
      bracket_structure = []
      current_list = []
      for dim in dims:
        if dim == '(':
          bracket_structure.append(current_list)
          current_list = []
        elif dim == ')':
          bracket_structure[-1].append(current_list)
          current_list = bracket_structure.pop()
        else:
          current_list.append(dim)
      return current_list

    src, dst = [parse_bracket_structure(parse_pattern(p)) for p in pattern.split('->')]
    ...
    if '...' in src:
      ...
      if '...' in dst:
        dst_idx = dst.index('...')
        n_dims = self.ndim - sum(isinstance(d, list) for d in src) + sum(len(d) - 1 for d in src if isinstance(d, list))
        ellipsis_dims = [f'_d{i}' for i in range(n_dims - len(src) + 1)]
        src = src[:idx] + ellipsis_dims + src[idx+1:]
        dst = dst[:dst_idx] + ellipsis_dims + dst[dst_idx+1:]
      else:
        raise ValueError("Ellipsis must be present in both source and destination patterns if used.")

    def flatten_dims(dims: List[str]) -> List[str]:
      return [d for dim in dims for d in (flatten_dims(dim) if isinstance(dim, list) else [dim])]

    flat_src, flat_dst = map(flatten_dims, (src, dst))

    # Compute shape
    shape: Dict[str, int] = {}
    for dim, length in zip(flat_src, self.shape):
      if dim in shape and shape[dim] != length:
        raise ValueError(f"Inconsistent size for dimension '{dim}': {shape[dim]} != {length}")
      shape[dim] = length

    shape.update(axes_lengths)

    for dim in set(flat_src + flat_dst):
      if dim not in shape:
        raise ValueError(f"Size for dimension '{dim}' is unknown")

    # Compute strides and sizes for reshape
    def compute_size_and_stride(dims: List[str]) -> Tuple[List[int], List[int]]:
      sizes, strides = [], []
      current_stride = 1
      for dim in reversed(dims):
        if isinstance(dim, list):
          sub_sizes, sub_strides = compute_size_and_stride(dim)
          sizes.extend(sub_sizes)
          strides.extend(sub_strides)
        else:
          sizes.append(shape[dim])
          strides.append(current_stride)
        current_stride *= (prod(sub_sizes) if isinstance(dim, list) else shape[dim])
      return list(reversed(sizes)), list(reversed(strides))

    src_sizes, src_strides = compute_size_and_stride(flat_src)
    dst_sizes, _ = compute_size_and_stride(flat_dst)

    # Reshape and transpose
    x = self.reshape(src_sizes)
    x = x.permute([src_strides.index(s) for s in sorted(src_strides, reverse=True)])
    return x.reshape(dst_sizes)
